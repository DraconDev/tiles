...
fn highlight_code<'a>(content: &'a str, language: &str) -> Vec<Line<'a>> {
    if language == "diff" {
        return content.lines().map(|line| {
            let style = if line.starts_with('+') && !line.starts_with("+++") {
                Style::default().fg(Color::Rgb(0, 255, 100)) // Matrix Green
            } else if line.starts_with('-') && !line.starts_with("---") {
                Style::default().fg(Color::Rgb(255, 0, 85)) // Neon Red
...
fn draw_file_view(f: &mut Frame, area: Rect, app: &mut App, pane_idx: usize, is_focused: bool, borders: Borders) {
    if let Some(pane) = app.panes.get_mut(pane_idx) {
        if let Some(preview) = &pane.preview {
            ...
            let highlighted = highlight_code(&preview.content, language);
    ...
    let mut display_path = if full_str.starts_with(&home) { full_str.replacen(&home, "~", 1) } else { full_str.to_string() };
                ...
                FileColumn::Size => { 
                    ...
                    let is_dir = metadata.map(|m| m.is_dir).unwrap_or(false);
                    let text = if is_dir && size == 0 { "<DIR>".to_string() } else { format_size(size) };
