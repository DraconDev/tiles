...
pub fn fs_mouse_index(row: u16, app: &App) -> usize {
    let mouse_row_offset = row.saturating_sub(3) as usize; // Header (1) + Tabs (1) + Breadcrumbs (1) = 3
...
pub fn navigate_back(fs: &mut FileState) {
...
pub fn navigate_forward(fs: &mut FileState) {
                ...
                if let Some(fs) = app.current_file_state() {
                    if let Some(path) = fs.files.get(*idx).cloned() {
                        let _ = event_tx.try_send(AppEvent::SpawnTerminal {
                            ...
                            command: Some(format!("./{}", path.file_name().unwrap().to_string_lossy())),
                        });
                    }
                }
            }
            ...
            if let ContextMenuTarget::File(idx) | ContextMenuTarget::Folder(idx) = target {
                if let Some(fs) = app.current_file_state() {
                    if let Some(path) = fs.files.get(*idx) {
                        ...
                        let _ = event_tx.try_send(AppEvent::StatusMsg(format!("Copied path to clipboard: {}", path_str)));
                    }
                }
            }
        }
                ...
                if let Some(fs) = app.current_file_state() {
                    if let Some(path) = fs.files.get(*idx) {
                        if let Some(name) = path.file_name() {
                            ...
                            let _ = event_tx.try_send(AppEvent::StatusMsg(format!("Copied name to clipboard: {}", name_str)));
                        }
                    }
                }
            }
        ...
        ContextMenuAction::TerminalWindow => {
            ...
            if let Some(path) = working_dir {
                let _ = event_tx.try_send(AppEvent::SpawnTerminal {
                    path,
                    new_tab: true,
                    remote: app.current_file_state().and_then(|fs| fs.remote_session.clone()),
                    command: None,
                });
            }
        }
            ...
            if let ContextMenuTarget::File(idx) | ContextMenuTarget::Folder(idx) = target {
                ...
                if let Some(path) = path_opt {
                    if let Some(name) = path.file_name() {
                        ...
                        tokio::spawn(async move {
                            crate::app::log_debug(&format!("Starting compression of {} to zip", name_str));
                            let _ = tx.try_send(AppEvent::TaskProgress(task_id, 0.0, format!("Compressing {}...", name_str)));
                            
                            let has_ark = std::process::Command::new("which").arg("ark").output().map(|o| o.status.success()).unwrap_or(false);
                            let has_file_roller = std::process::Command::new("which").arg("file-roller").output().map(|o| o.status.success()).unwrap_or(false);
                            let has_engrampa = std::process::Command::new("which").arg("engrampa").output().map(|o| o.status.success()).unwrap_or(false);
                            let has_zip = std::process::Command::new("which").arg("zip").output().map(|o| o.status.success()).unwrap_or(false);
                            
                            crate::app::log_debug(&format!("Tools found: ark={}, file-roller={}, engrampa={}, zip={}", has_ark, has_file_roller, has_engrampa, has_zip));
                            ...
                            let mut child_cmd = if has_ark {
                                ...
                                c.arg("-b").arg("-t").arg(&zip_name).arg(&filename).current_dir(&parent);
                                c
                                ...
                                c.arg("--add-to").arg(&zip_name).arg(&filename).current_dir(&parent);
                                c
                                ...
                                c.arg("-r").arg(&zip_name).arg(&filename).current_dir(&parent);
                                c
                            } else {
                                ...
                                c.arg("-czf").arg(&tar_name).arg(&filename).current_dir(&parent);
                                c
                            };

                            let mut child = match child_cmd.stdout(std::process::Stdio::piped()).stderr(std::process::Stdio::piped()).spawn() {
                                Ok(c) => c,
                                Err(e) => {
                                    let _ = tx.try_send(AppEvent::StatusMsg(format!("Failed to spawn tool: {}", e)));
                            ...
                            match res {
                                Ok(status) if status.success() => {
                                    let _ = tx.try_send(AppEvent::TaskProgress(task_id, 1.0, "Done".to_string()));
                                    sleep(Duration::from_millis(800)).await;
                                    let _ = tx.try_send(AppEvent::TaskFinished(task_id));
                                    let _ = tx.try_send(AppEvent::RefreshFiles(p_idx));
                                }
                                Ok(status) => {
                                    let _ = tx.try_send(AppEvent::StatusMsg(format!("Compression failed with status: {}", status)));
                                    let _ = tx.try_send(AppEvent::TaskFinished(task_id));
                                }
                                Err(e) => {
                                    let _ = tx.try_send(AppEvent::StatusMsg(format!("Error waiting for tool: {}", e)));
                                    let _ = tx.try_send(AppEvent::TaskFinished(task_id));
                                }
                            }
                        });
            ...
            if let ContextMenuTarget::File(idx) = target {
                ...
                if let Some(path) = path_opt {
                    ...
                    tokio::spawn(async move {
                        ...
                        let _ = tx.try_send(AppEvent::TaskProgress(task_id, 0.0, format!("Extracting {}...", filename)));
                        ...
                        let mut child_cmd = if has_ark {
                            ...
                            let mut c = TokioCommand::new("ark");
                            c.arg("-b").arg("-o").arg(parent.to_string_lossy().to_string()).arg(path.to_string_lossy().to_string());
                            c
                        } else if has_file_roller {
                            crate::app::log_debug("Using file-roller for extraction");
                            let mut c = TokioCommand::new("file-roller");
                            c.arg("--extract-to").arg(parent.to_string_lossy().to_string()).arg(path.to_string_lossy().to_string());
                            c
                        } else if has_engrampa {
                            ...
                            c.arg(path.to_string_lossy().to_string()).arg("-d").arg(parent.to_string_lossy().to_string());
                            c
                        } else if (ext == "tar" || ext == "gz" || ext == "xz" || ext == "bz2" || ext == "tgz") && has_tar {
                            crate::app::log_debug("Using tar for extraction");
                            let mut c = TokioCommand::new("tar");
                            c.arg("-xf").arg(path.to_string_lossy().to_string()).arg("-C").arg(parent.to_string_lossy().to_string());
                            c
                        } else if has_7z {
                            crate::app::log_debug("Using 7z for extraction");
                            let mut c = TokioCommand::new("7z");
                            c.arg("x").arg(path.to_string_lossy().to_string()).arg(format!("-o{}", parent.to_string_lossy()));
                            c
                        } else if has_tar {
                        ...
                        match res {
                            Ok(status) if status.success() => {
                                let _ = tx.try_send(AppEvent::TaskProgress(task_id, 1.0, "Extracted".to_string()));
                                sleep(Duration::from_millis(800)).await;
                                let _ = tx.try_send(AppEvent::TaskFinished(task_id));
                                let _ = tx.try_send(AppEvent::RefreshFiles(p_idx));
                            }
                            Ok(status) => {
                                let _ = tx.try_send(AppEvent::StatusMsg(format!("Extraction failed with status: {}", status)));
                                let _ = tx.try_send(AppEvent::TaskFinished(task_id));
                            }
                            Err(e) => {
                                let _ = tx.try_send(AppEvent::StatusMsg(format!("Error waiting for tool: {}", e)));
                ...
                tokio::spawn(async move {
                    ...
                    match res {
                        Ok(out) if out.status.success() => {
                            let _ = tx.try_send(AppEvent::StatusMsg("Initialized Git repository".to_string()));
                            let _ = tx.try_send(AppEvent::RefreshFiles(p_idx));
                        }
                        Ok(out) => {
                            let err = String::from_utf8_lossy(&out.stderr);
                            let _ = tx.try_send(AppEvent::StatusMsg(format!("Git init failed: {}", err)));
                        }
                        Err(e) => {
                            let _ = tx.try_send(AppEvent::StatusMsg(format!("Error running git: {}", e)));
                        }
                    }
                });
            }
                    ...
                    match res {
                        Ok(out) if out.status.success() => {
                            ...
                            if status.is_empty() {
                                ...
                                let _ = tx.try_send(AppEvent::StatusMsg(format!("Git: {}", status.trim().replace('\n', ", "))));
                            }
                        }
                        Ok(_) => {
                            let _ = tx.try_send(AppEvent::StatusMsg("Not a git repository".to_string()));
                        }
                        Err(e) => {
                            let _ = tx.try_send(AppEvent::StatusMsg(format!("Error checking git: {}", e)));
                        }
                    }
                });
            }
