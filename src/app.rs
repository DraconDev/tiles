...
impl App {
        ...
        if let Some(mut state) = crate::config::load_state() {
            ...
            if !state.panes.is_empty() {
                ...
                let mut app = Self {
                    ...
                    is_split_mode: state.is_split_mode,
                    terminal_size: (0, 0),
                    mouse_pos: (0, 0),
                    system_state,
                    license,
                    ...
                    ide_panel_tabs: vec![IdePanelTab::Search, IdePanelTab::Git, IdePanelTab::Log],
                    ide_active_panel_tab: 0,
                    ide_search_input: String::new(),
    ...
    pub fn move_up(&mut self, shift: bool) {
        ...
        if let Some(fs) = self.current_file_state_mut() {
            ...
            fs.selection.handle_move(i, shift);
    ...
    pub fn copy_to_other_pane(&mut self) {
        ...
        if let Some(fs) = self.current_file_state_mut() {
            let mut paths_to_copy: Vec<PathBuf> = Vec::new();
    ...
    pub fn move_to_other_pane(&mut self) {
        ...
        if let Some(fs) = self.current_file_state_mut() {
            let mut paths_to_move: Vec<PathBuf> = Vec::new();
            if !fs.selection.is_empty() {
                for &idx in fs.selection.multi_selected_indices() {
                    if let Some(p) = fs.files.get(idx) {
