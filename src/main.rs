                ...
                MouseEventKind::Down(button) => {
                    ...
                    if let AppMode::ContextMenu { x, y, target } = app.mode.clone() {
                        ...
                        if column >= draw_x && column < draw_x + menu_width && row >= draw_y && row < draw_y + menu_height {
                            ...
                            match target {
                                ContextMenuTarget::File(idx) => { if let Some(fs) = app.current_file_state_mut() { if let Some(path) = fs.files.get(idx).cloned() { match menu_row { 0 => { let _ = std::process::Command::new("xdg-open").arg(&path).spawn(); app.mode = AppMode::Normal; } 1 => { app.mode = AppMode::Rename; app.input = path.file_name().unwrap_or_default().to_string_lossy().to_string(); app.rename_selected = true; } 2 => app.mode = AppMode::Delete, 3 => app.mode = AppMode::Properties, _ => app.mode = AppMode::Normal, } } } } 
                                ContextMenuTarget::Folder(idx) => { if let Some(fs) = app.current_file_state_mut() { if let Some(path) = fs.files.get(idx).cloned() { match menu_row { 0 => { fs.current_path = path.clone(); fs.selected_index = Some(0); fs.search_filter.clear(); *fs.table_state.offset_mut() = 0; push_history(fs, path); let _ = event_tx.try_send(AppEvent::RefreshFiles(app.focused_pane_index)); app.mode = AppMode::Normal; } 1 => { if app.starred.contains(&path) { app.starred.retain(|x| x != &path); } else { app.starred.push(path.clone()); } app.mode = AppMode::Normal; } 2 => { app.mode = AppMode::Rename; app.input = path.file_name().unwrap_or_default().to_string_lossy().to_string(); app.rename_selected = true; } 3 => app.mode = AppMode::Delete, _ => app.mode = AppMode::Normal, } } } } 
