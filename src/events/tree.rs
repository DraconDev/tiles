...
fn expand_current(app: &mut App) {
    if app.tree_state.flat_items.is_empty() { return; }
                 ...
                 if app.tree_state.flat_items[i].depth == item.depth - 1 {
                     ...
                     // Ensure collapsed
                      if app.tree_state.flat_items[i].expanded {
                          app.tree_state.expanded_paths.remove(&app.tree_state.flat_items[i].path.clone());
                          refresh_tree(app);
                      }
                     return;
                 }
...
fn build_tree_recursive(path: &Path, depth: usize, expanded: &std::collections::HashSet<PathBuf>, acc: &mut Vec<TreeItem>) {
