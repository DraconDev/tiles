...
use terma::utils::{format_permissions, format_size, format_time, get_visual_width, squarify, truncate_to_width};
        ...
        CurrentView::Files => {
            ...
            for path in &app.starred {
                ...
                if is_selected {
                    ...
                    style = style.bg(Color::Rgb(50, 50, 100)).fg(Color::White);
                }
...
pub fn draw(f: &mut Frame, app: &mut App) {
    ...
    let is_editor_mode = matches!(app.mode, AppMode::Editor | AppMode::EditorSearch | AppMode::EditorGoToLine | AppMode::EditorReplace | AppMode::Viewer)
        || (matches!(app.mode, AppMode::Hotkeys) && matches!(app.previous_mode, AppMode::Editor | AppMode::EditorSearch | AppMode::EditorGoToLine | AppMode::EditorReplace | AppMode::Viewer));
    ...
    if is_editor_mode {
        ...
        let title = if let AppMode::EditorSearch = app.mode {
            Line::from(vec![
                Span::styled(" Find: ", Style::default().fg(border_color).add_modifier(Modifier::BOLD)),
                Span::styled(&app.input.value, Style::default().fg(Color::White)),
                Span::styled(" (Enter to Lock, Esc to Cancel) ", Style::default().fg(Color::DarkGray)),
            ])
        } else if let AppMode::EditorGoToLine = app.mode {
            Line::from(vec![
                Span::styled(" Go To Line: ", Style::default().fg(border_color).add_modifier(Modifier::BOLD)),
                Span::styled(&app.input.value, Style::default().fg(Color::White)),
            ])
        } else if let AppMode::EditorReplace = app.mode {
            if app.replace_buffer.is_empty() {
                Line::from(vec![
                    Span::styled(" Replace [FIND]: ", Style::default().fg(Color::Magenta).add_modifier(Modifier::BOLD)),
                    Span::styled(&app.input.value, Style::default().fg(Color::White)),
                    Span::styled(" (Tab/Enter to set Replace term) ", Style::default().fg(Color::DarkGray)),
                ])
            } else {
                Line::from(vec![
                    Span::styled(" Replace [WITH]: ", Style::default().fg(Color::Magenta).add_modifier(Modifier::BOLD)),
                    Span::styled(&app.input.value, Style::default().fg(Color::White)),
                    Span::styled(format!(" (Finding: '{}') [Enter: Next, ^Enter: All, Esc: Done] ", app.replace_buffer), Style::default().fg(Color::DarkGray)),
                ])
            }
        } else if let AppMode::Viewer = app.mode {
            Line::from(vec![
                Span::styled(" Viewer ", Style::default().fg(border_color).add_modifier(Modifier::BOLD)),
                Span::styled(format!("[{}]", status_text), Style::default().fg(border_color)),
                Span::raw(" │ "),
                Span::styled("Esc/Space Back", Style::default().fg(Color::Red)),
            ])
        } else {
            // Normal Editor Title with Status and Hotkeys
            Line::from(vec![
                Span::styled(" Editor ", Style::default().fg(border_color).add_modifier(Modifier::BOLD)),
                Span::styled(format!("[{}]", status_text), Style::default().fg(border_color)),
                Span::raw(" │ "),
                Span::styled("^F Find", Style::default().fg(Color::Yellow)),
                Span::raw(" │ "),
                Span::styled("^R Replace", Style::default().fg(Color::Yellow)),
                Span::raw(" │ "),
                Span::styled("^G Line", Style::default().fg(Color::Yellow)),
                Span::raw(" │ "),
    ...
    if let AppMode::DragDropMenu { ref source, ref target } = app.mode {
        draw_drag_drop_modal(f, app, source, target);
    }
}

fn draw_drag_drop_modal(f: &mut Frame, app: &App, source: &std::path::Path, target: &std::path::Path) {
    ...
    // Button bound logic (relative to inner.x, inner.y + 3)
    // [C] Copy (len 10) [M] Move (len 10) [Esc] Cancel (len 14)
    let (mx, my) = app.mouse_pos;
    let is_hover = |bx: u16, len: u16| {
        mx >= inner.x + bx && mx < inner.x + bx + len && my == inner.y + 3
    };

    let copy_style = if is_hover(0, 10) { Style::default().bg(Color::Green).fg(Color::Black) } else { Style::default().fg(Color::Green) };
    let move_style = if is_hover(12, 10) { Style::default().bg(Color::Yellow).fg(Color::Black) } else { Style::default().fg(Color::Yellow) };
    let cancel_style = if is_hover(24, 14) { Style::default().bg(Color::Red).fg(Color::Black) } else { Style::default().fg(Color::Red) };

    let text = vec![
        Line::from(vec![
            Span::raw("Item: "),
            Span::styled(src_name, Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
        ]),
        Line::from(vec![
            Span::raw("To:   "),
            Span::styled(truncate_to_width(&dest_path, (inner.width as usize).saturating_sub(6), "..."), Style::default().fg(Color::Cyan)),
        ]),
        Line::from(""),
        Line::from(vec![
...
fn draw_hotkeys_modal(f: &mut Frame, _area: Rect) {
    ...
    f.render_widget(
        Paragraph::new("Press ESC or F1 to Close").style(Style::default().fg(Color::DarkGray)).alignment(ratatui::layout::Alignment::Center),
        chunks[0]
    );
    ...
    for (section, items) in keys {
        rows.push(Row::new(vec![
            Cell::from(Span::styled(section, Style::default().fg(THEME.accent_primary).add_modifier(Modifier::BOLD))),
            Cell::from(""),
        ]));
        for (key, desc) in items {
            rows.push(Row::new(vec![
                Cell::from(Span::styled(format!("  {}", key), Style::default().fg(Color::Yellow))),
                Cell::from(desc),
            ]));
        }
...
fn draw_open_with_modal(f: &mut Frame, app: &App, path: &std::path::Path) {
    ...
    let (mx, my) = app.mouse_pos;
    let list_items: Vec<ListItem> = suggestions.iter().enumerate().map(|(i, s)| {
        let item_y = chunks[2].y + i as u16;
        let is_mouse_hovered = mx >= chunks[2].x && mx < chunks[2].x + chunks[2].width && my == item_y;
...
fn draw_file_view(
    ...
    if let Some(pane) = app.panes.get_mut(pane_idx) {
        if let Some(preview) = &pane.preview {
            ...
            let highlighted = terma::utils::highlight_code(&preview.content);
            let mut lines = Vec::new();
            for (i, line) in highlighted.iter().enumerate() {
                let mut spans = line.spans.clone();
                // Prepend line number gutter
                let num = format!("{:>3} │ ", i + 1);
                spans.insert(0, Span::styled(num, Style::default().fg(Color::Rgb(60, 60, 70))));
        ...
        .and_then(|p| p.current_state_mut())
    {
        ...
        *render_state.offset_mut() = file_state.table_state.offset();
        ...
        let search_filter_text = if !file_state.search_filter.is_empty() {
        ...
        for (i, comp) in components.iter().enumerate() {
            ...
            if !d_name.is_empty() {
                ...
                let is_drop_target =
                    matches!(&app.hovered_drop_target, Some(DropTarget::Folder(p)) if p == &s_path);
                let is_last = i == total_comps - 1;

                let fg_color = if is_drop_target {
                    Color::Rgb(0, 255, 200)
                } else if is_hovered {
                    Color::Rgb(255, 255, 0)
                } else if is_last {
                ...
                file_state.breadcrumb_bounds.push((bread_rect, s_path));
                cur_x += width;
        ...
        for i in 0..visible_height {
            ...
            let is_selected = file_state.selected_index == Some(file_idx);
            let is_multi_selected = file_state.multi_select.contains(&file_idx);

            let mut row_bg_style = Style::default();
            let is_hovered_drop = matches!(&app.hovered_drop_target, Some(DropTarget::Folder(p)) if p == path);
            ...
            if is_selected {
                ...
                row_bg_style = row_bg_style.bg(Color::Rgb(0, 255, 200));
                    ...
                    let content = match col_type {
                        FileColumn::Name => {
                            if path.to_string_lossy() == "__DIVIDER__" {
                                ...
                                if !is_selected && !app.path_colors.contains_key(path) {
                                    if is_dir {
                        ...
                        FileColumn::Size => {
                            if metadata.map(|m| m.is_dir).unwrap_or(false) {
                                ...
                                let text = format_size(metadata.map(|m| m.size).unwrap_or(0));
                                truncate_to_width(
                                    &text,
                                    (col_rect.width as usize).saturating_sub(2),
                                    "",
...
fn draw_footer(f: &mut Frame, area: Rect, app: &mut App) {
    ...
    if let Some(fs) = app.current_file_state() {
        let sel_count = if !fs.multi_select.is_empty() {
            fs.multi_select.len()
        } else if fs.selected_index.is_some() {
            1
        } else {
            0
        };
    ...
    // 4. Bottom Line: Background Tasks
    let mut task_spans = Vec::new();
    for task in &app.background_tasks {
        let pct = (task.progress * 100.0) as usize;
        let bar = "█".repeat(pct / 10) + &"░".repeat(10 - (pct / 10));
        task_spans.push(Span::styled(format!(" {} [{}%] ", task.name, pct), Style::default().fg(Color::Cyan)));
        task_spans.push(Span::styled(format!("{} ", bar), Style::default().fg(Color::Cyan)));
    }
...
fn draw_context_menu(
    ...
    for (i, action) in actions.iter().enumerate() {
        let label = match action {
            ...
            ContextMenuAction::Duplicate => format!(" {} Duplicate", Icon::Duplicate.get(app.icon_mode)),
            ...
            ContextMenuAction::TerminalWindow => {
                format!(" {} Open Terminal Window", Icon::Script.get(app.icon_mode))
            }
            ContextMenuAction::Refresh => format!(" {} Refresh", Icon::Refresh.get(app.icon_mode)),
            ContextMenuAction::SelectAll => format!(" {} Select All", Icon::SelectAll.get(app.icon_mode)),
            ContextMenuAction::ToggleHidden => format!(" {} Toggle Hidden", Icon::ToggleHidden.get(app.icon_mode)),
            ContextMenuAction::ConnectRemote => {
                format!(" {} Connect", Icon::Remote.get(app.icon_mode))
            }
            ContextMenuAction::DeleteRemote => format!(" {} Delete Bookmark", Icon::Delete.get(app.icon_mode)),
    ...
    if app.rename_selected {
        let text = if let Some(idx) = app.input.value.rfind('.') {
            if idx > 0 {
                ...
                Line::from(vec![
                    Span::styled(stem_part, Style::default().bg(THEME.accent_primary).fg(Color::Black)),
                    Span::raw(ext_part),
                ])
            } else {
                Line::from(vec![Span::styled(
...
fn draw_shortcuts_settings(f: &mut Frame, area: Rect, _app: &App) {
    let shortcuts = vec![
        ...
        ("Terminal", vec![("Ctrl + n", "Open External Terminal")]),
    ];
...
fn draw_general_settings(f: &mut Frame, area: Rect, app: &App) {
    let items = vec![
        ...
        ListItem::new(format!(
            "[{}] Auto Save (a)",
            if app.auto_save { "x" } else { " " }
        )),
        ListItem::new(format!("Icon Mode: {:?} (i)", app.icon_mode)),
    ];

    let items: Vec<ListItem> = items.into_iter().enumerate().map(|(i, item)| {
        if i == app.settings_index && app.settings_section == SettingsSection::General {
            item.style(Style::default().bg(THEME.accent_primary).fg(Color::Black).add_modifier(Modifier::BOLD))
        } else {
