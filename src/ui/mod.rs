    ...
    match app.current_view {
        CurrentView::Files => {
            ...
            for (i, bookmark) in app.remote_bookmarks.iter().enumerate() {
                ...
                sidebar_items.push(ListItem::new(format!(" {} {}", icon, bookmark.name)).style(style));
                app.sidebar_bounds.push(SidebarBounds { y: current_y, index: current_idx, target: SidebarTarget::Remote(i) });
                current_y += 1;
            }
...
pub fn draw(f: &mut Frame, app: &mut App) {
    ...
    if matches!(app.mode, crate::app::AppMode::AddRemote(_)) { draw_add_remote_modal(f, app); }
}
...
fn draw_footer(f: &mut Frame, area: Rect, app: &mut App) {
    let chunks = Layout::default().direction(Direction::Horizontal).constraints([Constraint::Min(0), Constraint::Length(20), Constraint::Length(45)]).split(area);
    ...
    if left_spans.len() > 2 { left_spans.push(Span::raw("  ")); }
    let shortcuts = vec![
    ...
    f.render_widget(Paragraph::new(draw_stat_bar("MEM", mem_usage, 100.0, stats_layout[1].width, THEME.fg)).alignment(ratatui::layout::Alignment::Right), stats_layout[1]);
...
fn draw_settings_modal(f: &mut Frame, app: &App) {
    let area = terma::layout::centered_rect(80, 80, f.area());
    f.render_widget(Clear, area);
    f.render_widget(Block::default().title(" Settings ").borders(Borders::ALL), area);
    f.render_widget(Paragraph::new("Settings menu (placeholder)"), area.inner(ratatui::layout::Margin { vertical: 1, horizontal: 1 }));
}
